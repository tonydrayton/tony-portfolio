...
export const ShinyAnchor = React.forwardRef<HTMLAnchorElement, ShinyAnchorProps>(
	({ children, className, ...props }, ref) => {
		return (
			...
			>
				<span
					className="relative size-full text-sm tracking-normal text-[rgb(0,0,0,100%)] dark:font-light dark:text-[rgb(255,255,255,100%)] flex flex-row items-center gap-4 px-1"
					style={{
						maskImage:
							"linear-gradient(-75deg,hsl(var(--primary)) calc(var(--x) + 20%),transparent calc(var(--x) + 30%),hsl(var(--primary)) calc(var(--x) + 100%))",
					}}
