		...
		<div ref={ref} className="relative w-full max-w-7xl mx-auto pb-20">
			{data.map((item, index) => (
				...
				>
					<div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
						<div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-(--color-background) flex items-center justify-center">
						...
						<div className="flex flex-col md:pl-20">
							...
							<div className="hidden md:block">
								{item.skills.map((skill, index) => (
									<Badge key={index} className="m-1 transition-all duration-300" variant="outline">
			...
			>
				<motion.div
					...
					className="absolute inset-x-0 top-0  w-[2px] bg-linear-to-t from-neutral-800 via-neutral-400 to-transparent from-0% via-10% rounded-full"
				/>
			</div>
		</div>
	);
